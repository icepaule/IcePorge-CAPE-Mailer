...
- Phishing-Analyse für EML/MSG-Dateien mit Multi-Source Auth-Header-Parsing
...
Version: 2.4.2
Changelog v2.4.2:
- FIX: Lock-File verschoben nach /opt/cape-mailer/ (cape-User hat keine Schreibrechte in /var/run)
- FIX: NameError in generate_cape_html_report behoben (result war nicht definiert)
- FEATURE: Erweiterte CAPE-Reports mit:
  - OTX Threat Intelligence Sektion
  - MITRE ATT&CK Mapping
  - FOR610 RAG-Analyse (KI-gestützt)
  - Behavioral Sequence Summary
  - Ghidra Static Analysis (von ki01)
Changelog v2.4.1:
- FIX: Process Lock verhindert mehrfache Verarbeitung (OSINT dauert >2min, Timer 2min)
- FIX: MISP exportiert jetzt ALLE URLs/IPs/Domains (nicht nur verdächtige)
- FIX: IOC-Extraction zeigt alle IPs aus Routing-Chain (nicht nur Originating IP)
- IMPROVEMENT: Konsistente IOCs zwischen Report und MISP
- IMPROVEMENT: IPs aus Mail-Routing-Chain als separate Attributes
Changelog v2.4.0:
- FEATURE: MISP-Integration für Phishing-Mails
- Intelligente Trigger-Logik (verdict, risk_score, ollama_keywords)
- Dynamische Tags basierend auf Analyse (banker, typosquatting, credential-theft, ceo-fraud)
- TLP:AMBER, Distribution: Your org only
- Ollama-Bewertung als Event-Beschreibung
- IOC-Export: email-src, ip-src, url, domain, email-headers
- FIX: ai_analysis statt ollama_assessment (AI-Override funktioniert jetzt!)
- FIX: Robustes Regex für "Risikobewertung: HIGH" Pattern (mit Newlines)
Changelog v2.3.4:
- FIX: Robustes Try-Except statt Capability-Check (Axigen lügt über MOVE-Support!)
- 3-stufiger Fallback: MOVE → COPY+DELETE (bulk) → COPY+DELETE (einzeln)
- Besseres Logging: Zeigt welche Methode verwendet wird
- Inspiriert von: https://github.com/freescout-help-desk/freescout/issues/4313
Changelog v2.3.3:
- FIX: Axigen MOVE-Support fehlt → COPY+DELETE Fallback implementiert
- Capability-Check für MOVE-Support (automatische Erkennung)
- Expunge nach DELETE für saubere Mailbox
Changelog v2.3.2:
- FIX: IMAP Timeout während langer Verarbeitung (Mails werden NACH Verarbeitung verschoben)
- FIX: AI-Verdict Priorisierung (KI überschreibt jetzt Risk-Score mit erweiterten Keywords)
- Verbesserung: Bulk-Verschiebung mit Fallback auf Einzel-Verschiebung
Changelog v2.3.1:
- Mailbox-Management: Verarbeitete Mails werden nach "Processed" verschoben
- Ausgehende Mails werden automatisch im "Sent"-Ordner gespeichert
- ALLE Mails in INBOX werden verarbeitet (nicht nur ungelesene)
- Loop-Prevention: Mails von malware@mpauli.de werden nicht beantwortet
- Fallback auf "als gelesen markieren" falls Verschiebung fehlschlägt
Changelog v2.4.1:
- Multi-Source OSINT-Integration (OTX, AbuseIPDB, VirusTotal, URLScan, Google Safe Browsing, Spamhaus, IPQualityScore)
- OSINT-Ergebnisse in Ollama-Prompt für Context-Aware AI-Bewertung
- HTML-Report mit OSINT-Threat-Intelligence-Sektion
- Threat-Score-Aggregation über alle OSINT-Quellen
Changelog v2.2.9:
...
- Security-Recommendations für Mail-Gateway-Tuning
- ARC-Header-Support
...
import subprocess
import urllib3
import fcntl  # v2.4.1: Process Lock
...
class PhishingAnalysisResult:
    ...
    osint_analysis: OSINTAnalysisResult = field(default_factory=OSINTAnalysisResult)  # NEU v2.4.1
    ai_analysis: str = ""
...
class ImprovedHeaderAnalyzer:
    ...
    def _analyze_authentication_multi_source(self):
        ...
        if dkim_result == "none":
            ...
            if dkim_sig:
                ...
                dkim_result = "unsigned"  # Marker für "nicht validiert"
        ...
        logger.debug(f"✅ Auth-Parsing: SPF={spf_result} ({spf_source}), DKIM={dkim_result} ({dkim_source}), DMARC={dmarc_result} ({dmarc_source})")

    def _analyze_received_chain(self):
        """Analysiere Received-Header-Kette (Legacy für IOC-Extraktion)"""
    ...
    def _generate_recommendations(self):
        """
        NEU: Generiere Security-Recommendations für Mail-Gateway-Tuning
        """
        recommendations = []
...
class OSINTAnalyzer:
    ...
    def analyze_all(self, ips: List[str], domains: List[str], urls: List[str]) -> OSINTAnalysisResult:
        """Führe vollständige OSINT-Analyse durch"""
        ...
        logger.info(f"🔍 OSINT-Analyse: {len(ips)} IPs, {len(domains)} Domains, {len(urls)} URLs")
        ...
        logger.info(f"✅ OSINT: {result.total_threat_indicators} Threats, Level={result.overall_threat_level}")
    ...
    def analyze_ip(self, ip: str) -> Optional[OSINTIPReputation]:
        ...
        try:
            ...
            logger.debug(f"IP-OSINT fehlgeschlagen für {ip}: {e}")
            return None
...
Begründe deine Einschätzung kurz."""
    ...
    def analyze_eml(self, eml_path: str) -> PhishingAnalysisResult:
        ...
        # Prüfe auf eingebettete message/rfc822
        embedded_msg = self._extract_embedded_message(msg)
        if embedded_msg:
            logger.info("📧 Eingebettete message/rfc822 gefunden - analysiere innere Mail")
            msg = embedded_msg
        ...
        if self.cfg.get('osint', {}).get('otx', {}).get('enabled') or \
           ...
           self.cfg.get('osint', {}).get('spamhaus', {}).get('enabled', True):
            logger.info("Führe OSINT-Analyse durch...")
    ...
    def _extract_body(self, msg) -> Tuple[str, str]:
        ...
        if msg.is_multipart():
            for part in msg.walk():
                ...
                # Überspringe message/rfc822 Parts (bereits extrahiert)
                if content_type == 'message/rfc822':
                    continue
    ...
    def _get_ai_analysis(self, result: PhishingAnalysisResult) -> str:
        ...
        try:
            ...
            logger.info(f"✅ Ollama-Analyse erfolgreich ({len(response)} Zeichen)")
            return response

        except requests.exceptions.ConnectionError as e:
            logger.error(f"❌ Ollama nicht erreichbar ({self.ollama_host}): {e}")
            return f"Ollama Connection Error: {e}"
        except requests.exceptions.Timeout as e:
            logger.error(f"❌ Ollama Timeout (>{self.ollama_timeout}s): {e}")
    ...
    def _calculate_overall_score(self, result: PhishingAnalysisResult):
        ...
        # Risk-Score basierte Bewertung (Baseline)
        ...
        # KRITISCH: KI-Bewertung überschreibt Risk-Score!
        if hasattr(result, 'ai_analysis') and result.ai_analysis:
            ai_lower = result.ai_analysis.lower()
            
            # 1. Prüfe EXPLIZIT auf Risikobewertung-Pattern (HÖCHSTE PRIORITÄT)
            import re
            
            # Pattern erlaubt Newlines und Markdown-Formatierung
            risk_pattern = (
                re.search(r'risikobewertung:\s*(high|critical|medium|low)', ai_lower) or
                re.search(r'risikobewertung:.*?[\n\r]+\s*\*?\*?(high|critical|medium|low)', 
                          ai_lower, re.DOTALL)
            )
            
            if risk_pattern:
                risk_level = risk_pattern.group(1)
                if risk_level in ['high', 'critical']:
                    if result.verdict != "malicious":
                        logger.info(f"🤖 AI-Override: {result.verdict.upper()} → MALICIOUS (Risikobewertung: {risk_level.upper()})")
                    result.verdict = "malicious"
                elif risk_level == 'medium':
                    if result.verdict in ["likely_clean", "clean"]:
                        logger.info(f"🤖 AI-Override: {result.verdict.upper()} → SUSPICIOUS (Risikobewertung: MEDIUM)")
                    result.verdict = "suspicious"
                # LOW wird ignoriert, Risk-Score entscheidet
            else:
                # 2. Fallback: Keyword-Suche für andere Phishing-Indikatoren
                malicious_keywords = [
                    'malicious', 'phishing', 'dangerous', 'threat', 'scam', 
                    'fraud', 'credential theft', 'credential harvesting', 'spoofing',
                    'typosquatting', 'fake login', 'fake website', 'social engineering',
                    'highly suspicious', 'clear phishing', 'definite phishing'
                ]
                
                suspicious_keywords = [
                    'suspicious', 'questionable', 'unusual', 'concerning',
                    'potentially malicious', 'red flags', 'warning signs'
                ]
                
                if any(keyword in ai_lower for keyword in malicious_keywords):
                    if result.verdict != "malicious":
                        logger.info(f"🤖 AI-Override: {result.verdict.upper()} → MALICIOUS (Keywords)")
                    result.verdict = "malicious"
                elif any(keyword in ai_lower for keyword in suspicious_keywords) and result.verdict in ["likely_clean", "clean"]:
                    logger.info(f"🤖 AI-Override: {result.verdict.upper()} → SUSPICIOUS (Keywords)")
                    result.verdict = "suspicious"


    def _extract_iocs(self, result: PhishingAnalysisResult) -> Dict:
        """Extrahiere IOCs für MISP - ALLE URLs/IPs/Domains (v2.4.1)"""
        
        # Alle URLs (nicht nur verdächtige)
        all_urls = [u.url for u in result.url_analyses if u.url]
        
        # Alle Domains (nicht nur Typosquatting)
        all_domains = list(set([u.domain for u in result.url_analyses if u.domain]))
        
        # Alle IPs sammeln
        all_ips = []
        
        # Originating IP
        if result.header_analysis.originating_ip and result.header_analysis.originating_ip not in ['Nicht ermittelt', 'N/A', '']:
            all_ips.append(result.header_analysis.originating_ip)
        
        # IPs aus Received-Chain
        for hop in result.header_analysis.received_chain:
            for ip in hop.get('ips', []):
                if ip and ip not in ['N/A', 'Nicht ermittelt', ''] and ip not in all_ips:
                    all_ips.append(ip)
...
# =============================================================================
# MISP PHISHING INTEGRATION
# =============================================================================

def create_misp_phishing_event(cfg: dict, result, subject: str) -> bool:
    """
    Erstellt MISP Event für Phishing-Mail
    
    Trigger-Logik:
    - verdict in ['suspicious', 'malicious'] ODER
    - risk_score >= trigger.min_risk_score ODER
    - Ollama-Keywords (high, critical, phishing, etc.)
    """
    misp_cfg = cfg.get('misp_phishing', {})
    
    if not misp_cfg.get('enabled', False):
        return False
    
    # Trigger prüfen
    trigger_cfg = misp_cfg.get('trigger', {})
    min_verdict = trigger_cfg.get('min_verdict', 'suspicious')
    min_risk_score = trigger_cfg.get('min_risk_score', 40)
    ollama_keywords = trigger_cfg.get('ollama_keywords', ['high', 'critical', 'phishing'])
    
    # Verdict-Check
    verdict_levels = ['clean', 'likely_clean', 'suspicious', 'malicious']
    verdict_trigger = verdict_levels.index(min_verdict) if min_verdict in verdict_levels else 2
    verdict_actual = verdict_levels.index(result.verdict) if result.verdict in verdict_levels else 0
    
    trigger_met = False
    trigger_reason = []
    
    if verdict_actual >= verdict_trigger:
        trigger_met = True
        trigger_reason.append(f"verdict={result.verdict}")
    
    if result.overall_risk_score >= min_risk_score:
        trigger_met = True
        trigger_reason.append(f"risk_score={result.overall_risk_score}")
    
    if hasattr(result, 'ai_analysis') and result.ai_analysis:
        ai_lower = result.ai_analysis.lower()
        for keyword in ollama_keywords:
            if keyword.lower() in ai_lower:
                trigger_met = True
                trigger_reason.append(f"ollama_keyword={keyword}")
                break
    
    if not trigger_met:
        logger.debug(f"📊 MISP: Trigger nicht erfüllt (verdict={result.verdict}, risk={result.overall_risk_score})")
        return False
    
    logger.info(f"🔔 MISP-Trigger erfüllt: {', '.join(trigger_reason)}")
    ...
    try:
        ...
        misp = PyMISP(
            url=misp_cfg['url'],
            key=misp_cfg['api_key'],
            ssl=misp_cfg.get('verify_ssl', False)
        )
        
        # Event erstellen
        event = MISPEvent()
        event.info = f"[Phishing] {subject[:100]}"
        event.distribution = misp_cfg.get('distribution', 0)  # Your org only
        event.analysis = misp_cfg.get('analysis', 1)  # Ongoing
        
        # Threat Level dynamisch
        if result.verdict == 'malicious':
            event.threat_level_id = 1  # High
        elif result.verdict == 'suspicious':
            event.threat_level_id = 2  # Medium
        else:
            event.threat_level_id = misp_cfg.get('threat_level_id', 2)
        
        # Ollama-Bewertung als Beschreibung
        if hasattr(result, 'ai_analysis') and result.ai_analysis:
            event.add_attribute('comment', result.ai_analysis[:2000], comment='Ollama AI Analysis')
        
        # Standard-Tags
        for tag in misp_cfg.get('tags', []):
            event.add_tag(tag)
        
        # Dynamische Tags
        dynamic_tags = misp_cfg.get('dynamic_tags', {})
        
        if dynamic_tags.get('banker') and any(kw in result.ai_analysis.lower() if hasattr(result, 'ai_analysis') else '' 
                                               for kw in ['bank', 'banking', 'iban', 'swift']):
            event.add_tag('banker')
        
        if dynamic_tags.get('typosquatting') and any(u.is_typosquatting for u in result.url_analyses):
            event.add_tag('typosquatting')
        
        if dynamic_tags.get('credential_harvesting') and 'credential' in (result.ai_analysis.lower() if hasattr(result, 'ai_analysis') else ''):
            event.add_tag('credential-theft')
        
        if dynamic_tags.get('ceo_fraud') and len(result.content_analysis.ceo_fraud_indicators) > 0:
            event.add_tag('social-engineering:technique="ceo-fraud"')
        
        # IOCs hinzufügen
        export_cfg = {
            'email_src': misp_cfg.get('export_email_src', True),
            'ip_src': misp_cfg.get('export_ip_src', True),
            'urls': misp_cfg.get('export_urls', True),
            'domains': misp_cfg.get('export_domains', True),
            'headers': misp_cfg.get('export_email_headers', True)
        }
        
        attribute_to_ids = misp_cfg.get('attribute_to_ids', False)
        
        # Email Source
        if export_cfg['email_src'] and result.header_analysis.from_address:
            event.add_attribute('email-src', result.header_analysis.from_address, 
                              to_ids=attribute_to_ids, comment='Phishing sender')
        
        # IP-Adressen (v2.4.1: Alle IPs inkl. Routing-Chain)
        if export_cfg['ip_src']:
            # Originating IP (falls vorhanden)
            if result.header_analysis.originating_ip and result.header_analysis.originating_ip not in ['Nicht ermittelt', 'N/A', '']:
                event.add_attribute('ip-src', result.header_analysis.originating_ip,
                                  to_ids=attribute_to_ids, comment='Originating IP')
            
            # Alle IPs aus Mail-Routing-Chain
            processed_ips = set()
            for hop in result.header_analysis.received_chain:
                for ip in hop.get('ips', []):
                    if ip and ip not in ['N/A', 'Nicht ermittelt', ''] and ip not in processed_ips:
                        event.add_attribute('ip-src', ip,
                                          to_ids=False, comment='Mail routing IP')
                        processed_ips.add(ip)
        
        # URLs (v2.4.1: ALLE URLs, nicht nur verdächtige)
        if export_cfg['urls']:
            for url_analysis in result.url_analyses:
                if url_analysis.url:  # Alle URLs exportieren
                    event.add_attribute('url', url_analysis.url, 
                                      to_ids=attribute_to_ids, 
                                      comment=f'Risk: {url_analysis.url_risk_score}/100')
        
        # Domains (v2.4.1: ALLE Domains, nicht nur Typosquatting)
        if export_cfg['domains']:
            processed_domains = set()
            for url_analysis in result.url_analyses:
                if url_analysis.domain and url_analysis.domain not in processed_domains:
                    comment = 'Typosquatting detected' if url_analysis.is_typosquatting else 'Domain from email'
                    event.add_attribute('domain', url_analysis.domain,
                                      to_ids=attribute_to_ids if url_analysis.is_typosquatting else False,
                                      comment=comment)
                    processed_domains.add(url_analysis.domain)
        
        # Email Headers
        if export_cfg['headers']:
            if result.header_analysis.reply_to and result.header_analysis.reply_to != result.header_analysis.from_address:
                event.add_attribute('email-reply-to', result.header_analysis.reply_to,
                                  to_ids=False, comment='Suspicious Reply-To')
            
            if result.header_analysis.return_path:
                event.add_attribute('email-src', result.header_analysis.return_path,
                                  to_ids=False, comment='Return-Path')
        
        # Event zu MISP hochladen
        response = misp.add_event(event, pythonify=True)
        
        if response and hasattr(response, 'id'):
            logger.info(f"✅ MISP Event erstellt: Event #{response.id} - {event.info}")
            return True
        else:
            logger.error(f"❌ MISP Event-Erstellung fehlgeschlagen: {response}")
            return False
            
    except ImportError:
        logger.error("❌ PyMISP nicht installiert! Führe aus: pip install pymisp --break-system-packages")
        return False
    except Exception as e:
        logger.error(f"❌ MISP-Fehler: {e}")
        import traceback
        logger.debug(traceback.format_exc())
        return False

# =============================================================================
# CAPE INTEGRATION (unverändert)
# =============================================================================
...
def submit_to_cape(cfg: dict, sample_path: str) -> Optional[int]:
    ...
    try:
        ...
        logger.info(f"📤 CAPE Submit: {os.path.basename(sample_path)} (route={route}, timeout={timeout})")
        logger.debug(f"Command: {' '.join(cmd_args)}")
        
        # User-Switch für subprocess
        ...
        result = subprocess.run(
            ...
            env={
                **os.environ,
                "PATH": "/opt/CAPEv2/.venv/bin:" + os.environ.get("PATH", ""),
                "VIRTUAL_ENV": "/opt/CAPEv2/.venv",
                "PYTHONPATH": "/opt/CAPEv2",
                "CAPE_AS_ROOT": "1",  # <-- CAPE erlaubt root!
                "USER": "cape"  # <-- Fake cape user
            }
        )
        ...
        if result.returncode == 0:
            ...
            if match:
                ...
                logger.error(f"❌ Keine Task ID gefunden in Output: {result.stdout}")
                return None
        else:
            logger.error(f"❌ CAPE Submit fehlgeschlagen (rc={result.returncode})")
...
def wait_for_cape_report(cfg: dict, task_id: int) -> Optional[Dict]:
    ...
    logger.warning(f"⚠️ Timeout beim Warten auf Report für Task #{task_id}")
    return None
...
""".strip()
    ...
    logger.info(f"🤖 Starte ITSO L3 Ollama-Analyse mit Modell {model}")
    ...
    try:
        ...
        response = r.json().get("response", "").strip()
        if response:
            logger.info("✅ Ollama ITSO L3 Analyse erfolgreich")
            return response
...
def generate_html_report(result: PhishingAnalysisResult) -> str:
    """Generiere HTML-Report für Phishing-Analyse mit Security-Recommendations"""
    ...
    for hop in routing.hops:
        ...
        <tr>
            <td>{hop.index}</td>
            <td>{hop.from_server or 'N/A'}</td>
            <td>{hop.to_server or 'N/A'}</td>
            <td>{', '.join(hop.ips) if hop.ips else 'N/A'}</td>
            <td>{hop.tls_version or 'N/A'}</td>
            <td>{hop.cipher[:30] if hop.cipher else 'N/A'}</td>
        </tr>
        """

    # Security-Recommendations aufbereiten
    recommendations_html = ""
    for rec in routing.recommendations:
        severity_color = {'high': '#dc3545', 'medium': '#ffc107', 'low': '#28a745'}.get(rec['severity'], '#6c757d')
        recommendations_html += f"""
        <div class="recommendation" style="border-left: 4px solid {severity_color}; margin: 10px 0; padding: 10px; background: #f8f9fa;">
            <strong style="color: {severity_color};">{rec['system']}</strong>
            <p><strong>Issue:</strong> {rec['issue']}</p>
            <p><strong>Recommendation:</strong> {rec['recommendation']}</p>
            <pre style="background: white; padding: 8px; font-size: 11px; overflow-x: auto;">
{chr(10).join([f"{k}: {v}" for k, v in rec.items() if k.startswith('config_')])}
            </pre>
        </div>
        """
...
<div class="container">
    ...
    <table>
        ...
        {routing_hops_html or '<tr><td colspan="6"><em>Keine Routing-Info</em></td></tr>'}
    </table>
    <p><strong>TLS Encryption:</strong> {'✅ Ja' if routing.uses_tls else '❌ Nein'} | 
       <strong>ARC Headers:</strong> {'✅ Ja' if routing.has_arc else '❌ Nein'}</p>
    {'<p class="risk-high"><strong>⚠️ Security Issues:</strong> ' + ', '.join(routing.security_issues) + '</p>' if routing.security_issues else ''}
    ...
    {recommendations_html or '<p><em>Keine Empfehlungen - System optimal konfiguriert!</em></p>'}
    ...
    <h2>🔍 OSINT Threat Intelligence <span class="info-badge">v2.4.1</span></h2>
    ...
    {f'<div class="section"><strong>⚠️ High-Confidence Threats:</strong><br>' + '<br>'.join([f'<span class="tag tag-danger">{t}</span>' for t in result.osint_analysis.high_confidence_threats]) + '</div>' if result.osint_analysis.high_confidence_threats else ''}
    ...
    <table>
        ...
        {''.join([f'<tr><td style="word-break:break-all;">{u.url[:60]}...</td><td class="{"risk-high" if u.overall_threat_score > 60 else "risk-medium" if u.overall_threat_score > 30 else ""}">{u.overall_threat_score}/100</td><td>{"".join([f"<span class=\"tag\">{t}</span>" for t in u.threat_tags]) or "<em>Clean</em>"}</td></tr>' for u in result.osint_analysis.url_reputations[:10]]) or '<tr><td colspan="3"><em>Keine URLs analysiert</em></td></tr>'}
    </table>
    ...
    <p style="color:#666; font-size:12px;">
        Generiert von CAPE Phishing Analyzer v2.4.1 (OSINT Integration) (Multi-Source Auth + Routing-Analyse)<br>
...
# =============================================================================
# CAPE REPORT HELPER FUNCTIONS (v2.4.2)
# =============================================================================

def generate_otx_section(report: Dict) -> str:
    """Generiere OTX Threat Intelligence Sektion"""
    otx = report.get('otx', {})
    if not otx or not otx.get('enabled'):
        return ""

    summary = otx.get('summary', {})
    hashes = otx.get('hashes', [])
    ips = otx.get('ips', [])
    domains = otx.get('domains', [])

    # Sammle alle Pulses
    all_pulses = []
    for h in hashes:
        if h.get('pulses'):
            all_pulses.extend(h['pulses'])
    for ip in ips:
        if ip.get('pulses'):
            all_pulses.extend(ip['pulses'])
    for d in domains:
        if d.get('pulses'):
            all_pulses.extend(d['pulses'])

    # Dedupliziere Pulses nach Name
    seen = set()
    unique_pulses = []
    for p in all_pulses:
        name = p.get('name', '')
    ...
    if not unique_pulses:
        return """
    <h2>🛡️ OTX Threat Intelligence</h2>
    <p style="color:#28a745;"><em>✅ Keine bekannten Bedrohungen in AlienVault OTX gefunden.</em></p>
    """

    pulse_html = ""
    for p in unique_pulses[:15]:
        pulse_html += f"""<tr>
            <td><strong>{p.get('name', 'Unknown')[:50]}</strong></td>
            <td>{p.get('author_name', 'Unknown')}</td>
            <td>{p.get('created', '')[:10]}</td>
            <td>{', '.join(p.get('tags', [])[:5])}</td>
        </tr>"""

    return f"""
    <h2>🛡️ OTX Threat Intelligence</h2>
    <p style="color:#dc3545;"><strong>⚠️ {len(unique_pulses)} Threat Intelligence Pulse(s) gefunden!</strong></p>
    <table>
        <tr><th>Pulse Name</th><th>Author</th><th>Erstellt</th><th>Tags</th></tr>
        {pulse_html}
    </table>
    """


def generate_mitre_section(report: Dict) -> str:
    """Generiere MITRE ATT&CK Sektion"""
    ttps = report.get('ttps', [])
    mitre = report.get('mitre_attck', {})

    if not ttps and not mitre:
        return ""
    ...
    for ttp in ttps[:20]:
        ...
        ttp_html += f'<span style="display:inline-block;padding:3px 8px;margin:2px;background:#e9ecef;border-radius:3px;font-size:11px;">{ttp_id}</span>'

    return f"""
    <h2>🎯 MITRE ATT&CK Mapping</h2>
    <div style="background:#f8f9fa;padding:15px;border-radius:5px;">
        {ttp_html or '<em>Keine MITRE ATT&CK Techniken erkannt</em>'}
    </div>
    """


def generate_for610_section(task_id: int) -> str:
    """Generiere FOR610 RAG Analysis Sektion"""
    rag_path = f"/opt/CAPEv2/storage/analyses/{task_id}/reports/rag_analysis.json"
    ...
    try:
        ...
        analysis_html = analysis.replace('### ', '<h4>').replace('\n\n', '</h4>\n<p>').replace('\n- ', '<br>• ')
        analysis_html = analysis_html.replace('**', '<strong>').replace('**', '</strong>')

        source_html = ""
        if sources:
            source_html = "<p style='font-size:11px;color:#666;'><strong>Quellen:</strong> "
            source_html += ", ".join([f"{s.get('source', '')} (S. {s.get('page', '')})" for s in sources[:8]])
            source_html += "</p>"

        return f"""
    <h2>📚 FOR610 Analyse (KI-gestützt)</h2>
    <div style="background:#f0f7ff;padding:15px;border-radius:5px;border-left:4px solid #0066cc;">
    ...
    """
        ...
        logger.debug(f"FOR610 Section: {e}")
        return ""


def generate_behavioral_section(task_id: int) -> str:
    """Generiere Behavioral Sequence Sektion (Link zum Report)"""
    beh_path = f"/opt/CAPEv2/storage/analyses/{task_id}/reports/behavioral_sequence.html"
    summary_path = f"/opt/CAPEv2/storage/analyses/{task_id}/reports/behavioral_summary.html"

    if not os.path.exists(beh_path) and not os.path.exists(summary_path):
        return ""

    # Lies Summary für Kurzübersicht
    try:
        if os.path.exists(summary_path):
            with open(summary_path, 'r') as f:
                summary_content = f.read()

            # Extrahiere Stats aus dem HTML
            import re
            api_match = re.search(r'<div class="stat-value">(\d+)</div>\s*<div class="stat-label">API Calls</div>', summary_content)
            suspicious_match = re.search(r'<div class="stat-value[^"]*">(\d+)</div>\s*<div class="stat-label">Verdächtig</div>', summary_content)

            api_calls = api_match.group(1) if api_match else "?"
            suspicious = suspicious_match.group(1) if suspicious_match else "?"

            return f"""
    <h2>🔬 Behavioral Sequence Analysis</h2>
    <div style="background:#fff3cd;padding:15px;border-radius:5px;border-left:4px solid #ffc107;">
        <p><strong>API Calls:</strong> {api_calls} | <strong>Verdächtige Aufrufe:</strong> {suspicious}</p>
        <p style="font-size:11px;">Detaillierte Verhaltensanalyse verfügbar in: <code>/opt/CAPEv2/storage/analyses/{task_id}/reports/</code></p>
    </div>
    """
    except Exception as e:
        logger.debug(f"Behavioral Section: {e}")

    return f"""
    <h2>🔬 Behavioral Sequence Analysis</h2>
    <p>Behavioral Reports verfügbar unter: <code>/opt/CAPEv2/storage/analyses/{task_id}/reports/</code></p>
    """


def generate_cape_osint_section(report: Dict, cfg: dict) -> str:
    """Generiere OSINT-Reputations-Sektion für CAPE-Samples"""
    try:
        network = report.get('network', {})
        ...
        if not ips and not domain_names:
            return ""

        # Initialisiere OSINT-Analyzer
        osint = OSINTAnalyzer(cfg)

        # Analysiere IPs und Domains
        ip_results = []
        domain_results = []

        for ip in ips[:10]:  # Max 10 IPs
            result = osint.analyze_ip(ip)
            if result:
                ip_results.append(result)

        for domain in domain_names[:10]:  # Max 10 Domains
            result = osint.analyze_domain(domain)
            if result:
                domain_results.append(result)

        # Berechne Overall Threat Level
        high_threats = []
        for ip in ip_results:
            if ip.overall_threat_score > 60:
                high_threats.append(f"IP {ip.ip}: Score {ip.overall_threat_score}")
        for domain in domain_results:
            if domain.overall_threat_score > 60:
                high_threats.append(f"Domain {domain.domain}: Score {domain.overall_threat_score}")

        threat_level = "critical" if len(high_threats) > 3 else "high" if len(high_threats) > 0 else "medium" if any(ip.overall_threat_score > 30 for ip in ip_results) or any(d.overall_threat_score > 30 for d in domain_results) else "low"

        threat_color = '#dc3545' if threat_level in ['critical', 'high'] else '#ffc107' if threat_level == 'medium' else '#28a745'

        # IP-Tabelle
        ip_html = ""
        for ip in ip_results[:10]:
            score_class = 'risk-high' if ip.overall_threat_score > 60 else 'risk-medium' if ip.overall_threat_score > 30 else ''
            tags_html = ''.join([f'<span style="display:inline-block;padding:2px 5px;margin:1px;background:#fee;border-radius:3px;font-size:10px;">{t}</span>' for t in ip.threat_tags[:5]]) or '<em style="color:#28a745;">Clean</em>'
            ip_html += f"<tr><td>{ip.ip}</td><td class='{score_class}'>{ip.overall_threat_score}/100</td><td>{tags_html}</td></tr>"

        # Domain-Tabelle
        domain_html = ""
        for d in domain_results[:10]:
            score_class = 'risk-high' if d.overall_threat_score > 60 else 'risk-medium' if d.overall_threat_score > 30 else ''
            tags_html = ''.join([f'<span style="display:inline-block;padding:2px 5px;margin:1px;background:#fee;border-radius:3px;font-size:10px;">{t}</span>' for t in d.threat_tags[:5]]) or '<em style="color:#28a745;">Clean</em>'
            domain_html += f"<tr><td>{d.domain}</td><td class='{score_class}'>{d.overall_threat_score}/100</td><td>{tags_html}</td></tr>"

        high_threats_html = ""
        if high_threats:
            high_threats_html = f"""
        <div style="background:#fee;padding:10px;border-radius:5px;margin-bottom:15px;">
            <strong>⚠️ High-Confidence Threats:</strong><br>
            {'<br>'.join(high_threats[:10])}
        </div>
        """

        return f"""
    <h2>🔍 OSINT Threat Intelligence</h2>
    <div class="verdict" style="border-left:5px solid {threat_color};padding:10px;background:#f8f9fa;">
        <strong>Threat Level: {threat_level.upper()}</strong>
        <span style="float:right;">{len(high_threats)} High-Confidence Threats</span>
    </div>

    {high_threats_html}

    <h3>🌐 IP Reputation ({len(ip_results)} IPs)</h3>
    <table>
        <tr><th>IP</th><th>Threat Score</th><th>Threat Indicators</th></tr>
        {ip_html or '<tr><td colspan="3"><em>Keine IPs analysiert</em></td></tr>'}
    </table>

    <h3>🏷️ Domain Reputation ({len(domain_results)} Domains)</h3>
    <table>
        <tr><th>Domain</th><th>Threat Score</th><th>Threat Indicators</th></tr>
        {domain_html or '<tr><td colspan="3"><em>Keine Domains analysiert</em></td></tr>'}
    </table>
    """
    except Exception as e:
        logger.debug(f"OSINT Section Error: {e}")
        return ""


def generate_ghidra_section(report: Dict) -> str:
    """Generiere Ghidra Analysis Sektion (von ki01)"""
    # Versuche Ghidra-Analyse von ki01 zu holen
    try:
        sample_info = report.get('target', {}).get('file', {})
        sha256 = sample_info.get('sha256', '')

        if not sha256:
            return ""

        # Prüfe ob Ghidra-Analyse existiert
        response = requests.get(
            f"http://10.10.0.219:5001/analysis/{sha256}",
            timeout=5
        )
        ...
        ghidra_data = response.json()

        if not ghidra_data.get('has_results'):
            return f"""
    <h2>🔍 Ghidra Statische Analyse</h2>
    <p style="color:#666;"><em>Ghidra-Analyse noch nicht abgeschlossen oder nicht verfügbar für {sha256[:16]}...</em></p>
    <p style="font-size:11px;">Analyse kann auf ki01 (10.10.0.219) angefordert werden.</p>
    """

        findings = ghidra_data.get('findings', [])
        analysis_text = ghidra_data.get('analysis', '')
    ...
    <div style="background:#e8f5e9;padding:15px;border-radius:5px;border-left:4px solid #4caf50;">
        <p><strong>Status:</strong> ✅ Analyse abgeschlossen</p>
        {f'<p><strong>Findings ({len(findings)}):</strong></p><ul style="font-size:12px;">{findings_html}</ul>' if findings else ''}
        {f'<details><summary>Detaillierte Analyse (klicken zum Aufklappen)</summary><pre style="font-size:11px;max-height:300px;overflow-y:auto;">{analysis_text[:3000]}</pre></details>' if analysis_text else ''}
    </div>
    """
    except requests.exceptions.RequestException:
        return ""
    except Exception as e:
        logger.debug(f"Ghidra Section: {e}")
        return ""


def generate_cape_html_report(sample_name: str, task_id: int, report: Dict,
                              ai_analysis: str, malscore: float, cfg: dict = None) -> str:
    """Generiere HTML-Report für CAPE-Analyse (v2.4.2 mit erweiterten Analysen)"""
...
<div class="container">
    ...
    {generate_otx_section(report)}

    {generate_cape_osint_section(report, cfg) if cfg else ''}

    {generate_mitre_section(report)}

    <h2>🤖 KI-Bewertung (Ollama)</h2>
    <pre>{ai_analysis}</pre>

    {generate_for610_section(task_id)}

    {generate_behavioral_section(task_id)}

    {generate_ghidra_section(report)}

    <hr>
    <p style="color:#666; font-size:12px;">CAPE Mailer v2.4.2 - Extended Analysis Report</p>
</div>
</body>
</html>"""
...
def save_to_sent_folder(cfg: dict, msg: EmailMessage) -> bool:
    """Speichere gesendete Mail im Sent-Ordner via IMAP"""
    ...
    try:
        ...
        with IMAPClient(imap_cfg["host"], port=port, ssl=use_ssl, ssl_context=ssl_context) as client:
            ...
            # Mail als Bytes für IMAP Upload
            msg_bytes = msg.as_bytes()
            
            # In Sent-Ordner speichern
            sent_folder = "Sent"
            client.append(sent_folder, msg_bytes, flags=['\\Seen'])
            logger.info(f"✅ Mail in '{sent_folder}' gespeichert")
            return True
            
    except Exception as e:
        logger.warning(f"⚠️ Konnte Mail nicht in Sent speichern: {e}")
        return False
...
def send_mail(cfg: dict, to_addr: str, subject: str, html_body: str,
    ...
    if not smtp_cfg.get("host"):
        logger.error("❌ SMTP host nicht konfiguriert!")
        return False
        
    if not smtp_cfg.get("port"):
        logger.error("❌ SMTP port nicht konfiguriert!")
        return False
    ...
    try:
        logger.debug(f"📧 Verbinde mit SMTP {smtp_cfg.get('host')}:{smtp_cfg.get('port')}")
        ...
        with smtplib.SMTP(smtp_cfg.get("host"), int(smtp_cfg.get("port", 587)), timeout=30) as server:
            logger.debug("✅ SMTP Verbindung hergestellt")
            
            if smtp_cfg.get("starttls", True):
                server.starttls(context=ssl_context)
                logger.debug("✅ STARTTLS erfolgreich")
            ...
            logger.info(f"✅ Mail erfolgreich gesendet an {to_addr}")
            
            # Mail im Sent-Ordner speichern
            save_to_sent_folder(cfg, msg)
            
            return True

    except ConnectionRefusedError as e:
        logger.error(f"❌ SMTP Server nicht erreichbar ({smtp_cfg.get('host')}:{smtp_cfg.get('port')}): {e}")
        logger.error("💡 Prüfe: sudo systemctl status postfix (oder exim4)")
        return False
    except smtplib.SMTPAuthenticationError as e:
        logger.error(f"❌ SMTP Authentifizierung fehlgeschlagen: {e}")
        logger.error(f"💡 User: {smtp_cfg.get('user')}")
        return False
    except socket.gaierror as e:
        logger.error(f"❌ SMTP Hostname nicht auflösbar ({smtp_cfg.get('host')}): {e}")
        return False
    except Exception as e:
...
# =============================================================================
# MAIN PROCESSING (unverändert)
# =============================================================================

def fetch_mailbox(cfg: dict) -> List[Dict]:
    """Hole ALLE Mails aus INBOX (unabhängig von gelesen/ungelesen Status)"""
    imap_cfg = cfg.get("imap", {})
    ...
    try:
        ...
        with IMAPClient(imap_cfg["host"], port=port, ssl=use_ssl, ssl_context=ssl_context) as client:
            ...
            logger.info(f"Gefunden: {len(uids)} E-Mail(s) in INBOX zur Verarbeitung")
...
def process_mail(cfg: dict, mail_data: Dict) -> bool:
    ...
    if eml_files and PHISHING_DEPS_AVAILABLE:
        ...
        for eml_file in eml_files:
            try:
                ...
                # Loop-Prevention: Nicht an eigene Adresse senden
                own_address = cfg.get("imap", {}).get("user", "")
                if from_addr.lower() == own_address.lower():
                    logger.info(f"ℹ️ Keine Antwort-Mail versendet (Loop-Prevention: from={from_addr})")
                    results_sent = True
                elif send_mail(cfg, from_addr, mail_subject, html_report, cc_addr=ALWAYS_CC):
                    results_sent = True
                else:
                    logger.warning("⚠️ Mail-Versand fehlgeschlagen, aber Report wurde gespeichert")
                    results_sent = True
                
                # MISP-Integration (v2.4.0)
                try:
                    misp_created = create_misp_phishing_event(cfg, result, subj)
                    if misp_created:
                        logger.info("📤 MISP Event erstellt")
                except Exception as misp_err:
                    logger.warning(f"⚠️ MISP-Integration fehlgeschlagen: {misp_err}")
    ...
    if samples:
        for sample in samples:
            ...
            html_report = generate_cape_html_report(sample_name, task_id, report, ai_analysis, malscore, cfg)
            report_path = os.path.join(REPORTS_DIR, f"cape_{task_id}_{sample_hash[:8]}.html")
            with open(report_path, 'w') as f:
                f.write(html_report)
            logger.info(f"💾 CAPE-Report gespeichert: {report_path}")
...
def acquire_process_lock():
    """
    Verhindert mehrfache Verarbeitung durch File-Lock
    Wichtig: OSINT-Analyse kann >2 Minuten dauern, Timer läuft alle 2 Minuten
    """
    # Lock-File im cape-mailer Verzeichnis (cape-User hat Schreibrechte)
    lock_file_path = os.path.join(BASE, 'cape-mailer.lock')
    ...
    try:
        ...
        # Schreibe PID in Lock-File
        lock_file.write(str(os.getpid()))
        lock_file.flush()

        return lock_file  # Lock-File-Handle zurückgeben (nicht schließen!)

    except BlockingIOError:
        # Lock bereits gehalten von anderem Prozess (fcntl.LOCK_NB)
        return None
    except IOError as e:
        # Andere I/O-Fehler - loggen und trotzdem fortfahren (kein Lock)
        logger.warning(f"⚠️ Lock-File konnte nicht erstellt werden: {e} - fahre ohne Lock fort")
        return "no_lock"  # Spezieller Wert: kein Lock, aber fortfahren
...
def main():
    """Hauptfunktion"""
    logger.info("=" * 60)
    logger.info("CAPE Mailer v2.4.2 - Extended Reports + Lock-Fix gestartet")
    
    # v2.4.1: Process Lock (verhindert mehrfache Verarbeitung)
    lock_handle = acquire_process_lock()
    if lock_handle is None:
        logger.info("⚠️ Anderer cape-mailer-Prozess läuft bereits - überspringe")
        logger.info("=" * 60)
        return
    elif lock_handle == "no_lock":
        logger.info("⚠️ Kein Lock möglich - fahre ohne Lock fort (PID: {})".format(os.getpid()))
    else:
        logger.info("✅ Process Lock erworben (PID: {})".format(os.getpid()))
    ...
    if ollama_ok:
        logger.info(f"✅ Ollama erreichbar: {cfg.get('ollama', {}).get('host')}")
    else:
        logger.warning("⚠️ Ollama nicht erreichbar")

    if cape_ok:
        logger.info("✅ CAPE API erreichbar")
    else:
        logger.warning("⚠️ CAPE API nicht erreichbar")

    mails = fetch_mailbox(cfg)

    if not mails:
        logger.info("📭 INBOX ist leer - keine Mails zur Verarbeitung")
        return
    ...
    # Sammle erfolgreich verarbeitete UIDs (AUSSERHALB der IMAP-Connection!)
    processed_uids = []
    
    # Verarbeite alle Mails OHNE offene IMAP-Connection (um Timeout zu vermeiden)
    for mail_data in mails:
        ...
        if has_samples and not cape_ok:
            logger.warning(f"⚠️ Mail #{uid} übersprungen - CAPE nicht verfügbar")
            continue
        try:
            success = process_mail(cfg, mail_data)
            if success:
                processed_uids.append(uid)
                logger.info(f"✅ Mail #{uid} erfolgreich verarbeitet")
            else:
                logger.warning(f"⚠️ Mail #{uid} - keine Ergebnisse generiert")
        except Exception as e:
            logger.error(f"❌ Fehler bei Mail #{uid}: {e}")
            import traceback
            logger.debug(traceback.format_exc())
    
    # Jetzt ALLE erfolgreichen Mails in einer NEUEN IMAP-Connection verschieben
    if processed_uids:
        logger.info(f"📦 Verschiebe {len(processed_uids)} verarbeitete Mail(s) nach 'Processed'")
        with IMAPClient(imap_cfg["host"], port=port, ssl=use_ssl, ssl_context=ssl_context) as client:
            if not use_ssl and imap_cfg.get("starttls", True):
                client.starttls(ssl_context=ssl_context)
            ...
            # Strategie: Erst MOVE versuchen, bei Fehler COPY+DELETE
            # Grund: Axigen zeigt MOVE in Capabilities, unterstützt es aber nicht!
            try:
                # Versuche MOVE (funktioniert bei den meisten Servern)
                client.move(processed_uids, 'Processed')
                logger.info(f"✅ {len(processed_uids)} Mail(s) nach 'Processed' verschoben (MOVE)")
            except Exception as move_err:
                # MOVE fehlgeschlagen → COPY+DELETE Fallback (für Axigen & buggy Server)
                logger.info(f"⚠️ MOVE fehlgeschlagen: {move_err}")
                logger.info(f"📋 Nutze COPY+DELETE Fallback...")
                try:
                    client.copy(processed_uids, 'Processed')
                    logger.info(f"✅ {len(processed_uids)} Mail(s) nach 'Processed' kopiert")
                    client.delete_messages(processed_uids)
                    client.expunge()
                    logger.info(f"✅ {len(processed_uids)} Mail(s) aus INBOX gelöscht")
                except Exception as bulk_err:
                    # Bulk-COPY fehlgeschlagen → Einzeln verarbeiten
                    logger.warning(f"⚠️ Bulk-COPY fehlgeschlagen: {bulk_err}")
                    logger.info("📋 Verarbeite Mails einzeln...")
                    success_count = 0
                    for uid in processed_uids:
                        try:
                            ...
                            try:
                                ...
                                logger.info(f"✅ Mail #{uid} verschoben (MOVE)")
                                success_count += 1
                            except:
                                # MOVE fehlgeschlagen → COPY+DELETE
                                client.copy([uid], 'Processed')
                                client.delete_messages([uid])
                                logger.info(f"✅ Mail #{uid} verschoben (COPY+DELETE)")
                                success_count += 1
                        except Exception as e:
                            logger.warning(f"⚠️ Mail #{uid}: {e} - markiere als gelesen")
                            try:
                                client.add_flags([uid], ['\\Seen'])
                            except:
                                pass
                    # Expunge nach allen Einzellöschungen
                    try:
                        client.expunge()
                        logger.info(f"✅ Expunge durchgeführt ({success_count}/{len(processed_uids)} Mails erfolgreich)")
                    except Exception as exp_err:
                        logger.debug(f"⚠️ Expunge fehlgeschlagen: {exp_err}")
